...
| Verified Shopify "Checkout Started" payload shape | ✅ | klaviyo-template-syntax-verified.md lines 37-58 | 2026-05-07 | `$value` is numeric (not string). `$extra.line_items` has full cart. `$extra.token` for cart recovery URLs. Customer-facing URL needs replace from myshopify.com → bargainchemist.co.nz, but parser bug prevents chained replace; use Shopify 301. |
