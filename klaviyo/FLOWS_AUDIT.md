...
- 2026-05-06 — Ran `--audit-action-statuses` to map every action's individual status. **Found only 6 LIVE email-send actions across all flows** (was assuming 24+). Most "critical" findings were on draft actions = false alarms. Real shipping production: Post-Purchase E1 (105627862), ATC E1 (98627502), Win-back E1 (105721759), Welcome E1 + E2 (105721561 + 105721565), Flu Season E1 (105627866). Filter bug excluded MJML actions in Y84ruV (Abandoned Checkout) and Ysj7sg (Back in Stock); fixed in next commit. Reconciliation: most of FLOWS_AUDIT.md's "🚨 critical" should be re-rated to "high but not currently shipping".
- 2026-05-06 — Read live-bound Post-Purchase templates Vc5vyk (E1) and U8ub97 (E2 draft) directly via MCP. **Discovered the Post-Purchase E1 + E2 templates are wired to the wrong action slots:** E1 (LIVE) sends "Running low?" replenishment content with broken `{ first_name }` (single-brace) variables; E2 (draft) has the review-request content (also with broken `{ first_name }`). Combined with subject overrides ("more to love" / "How did we do?") that don't match body content. The "review CTA → homepage" issue I flagged earlier is in E2's content, not E1.
...
- 2026-05-06 — Pulled comprehensive 180-day Shopify analytics: 6 queries covering top 100 by orders, top 60 by gross sales, top by returning customer rate, plus per-category drill-downs for Baby (2,096 orders), Cosmetics (5,035), Household (1,509), Personal Care (16,949), Skin Care (10,018). Hayfexo CONFIRMED as `_pharmacy-only` SKU (286 orders, $9.7k revenue) — same restricted-product class as Regaine. Allergy branch (action 105717138) paused.
- 2026-05-06 — **V1 RETAIL-FIRST REPLENISHMENT DEPLOYED.** All 15 retail-category templates POSTed to Klaviyo + flow-actions 105717126→105717169 rebound to the new content (preserving `definition.links`). Klaviyo cloned each library template internally to a flow-embedded copy on bind (verified content match for slot 16 SVHLXp = identical to library VwrdMh). All 15 actions remain in `draft` (or `manual` for slot 6 ex-Hayfexo) — no customer email ships until manually activated. V1 limits: all CTAs link to `/collections/all`, no product images. V2 work pending: per-slot collection URLs + product images + library cleanup.
